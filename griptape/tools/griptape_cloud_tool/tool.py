...
class GriptapeCloudToolTool(BaseGriptapeCloudTool):
    ...
    def _init_activities(self) -> None:
        ...
        for activity_name, (description, activity_schema) in activity_schemas.items():
            activity_handler = self._create_activity_handler(
                activity_name, description, activity_schema
            )
    ...
    def _get_schema(self) -> dict:
        response = requests.get(
            urljoin(self.base_url, f"/api/tools/{self.tool_id}/openapi"),
        ...
        if "error" in schema and "tool_run_id" in schema:
            raise RuntimeError(
                f"Failed to retrieve schema for tool {self.tool_id}: {schema['error']}"
            )
    ...
    def __extract_schema_from_ref(self, schema: dict, schema_ref: dict) -> Schema:
        ...
        for prop, prop_info in schema_data.get("properties", {}).items():
            ...
            properties[schema_prop] = self._map_openapi_type_to_python(
                prop_type, prop_info
            )
    ...
    def _map_openapi_type_to_python(
        ...
        if (
            openapi_type == "array"
            and schema_info is not None
            and "items" in schema_info
        ):
    ...
    def _create_activity_handler(
        self, activity_name: str, description: str, activity_schema: Schema
    ) -> Callable:
    ...
    def _run_activity(self, activity_name: str, params: dict) -> BaseArtifact:
        ...
        url = urljoin(
            self.base_url, f"/api/tools/{self.tool_id}/activities/{activity_name}"
        )
