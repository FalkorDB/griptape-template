...
class FileManagerTool(BaseTool):
    ...
    file_manager_driver: BaseFileManagerDriver = field(
        default=Factory(lambda: LocalFileManagerDriver()), kw_only=True
    )

    loaders: dict[str, loaders.BaseLoader] = field(
        default=Factory(
            lambda self: {
                "application/pdf": loaders.PdfLoader(
                    file_manager_driver=self.file_manager_driver
                ),
                "text/csv": loaders.CsvLoader(
                    file_manager_driver=self.file_manager_driver
                ),
                "text": loaders.TextLoader(
                    file_manager_driver=self.file_manager_driver
                ),
                "image": loaders.ImageLoader(
                    file_manager_driver=self.file_manager_driver
                ),
                "application/octet-stream": BlobLoader(
                    file_manager_driver=self.file_manager_driver
                ),
            },
            takes_self=True,
            ...
            "schema": Schema(
                {
                    Literal(
                        "path",
                        description="Relative path in the POSIX format. For example, 'foo/bar'",
                    ): str
                },
            ),
        },
    ...
    def save_memory_artifacts_to_disk(
        ...
        if not memory:
            return ErrorArtifact(
                f"Failed to save memory artifacts to disk - memory named '{memory_name}' not found"
            )
        ...
        for artifact in list_artifact.value:
            formatted_file_name = (
                f"{artifact.name}-{file_name}" if len(list_artifact) > 1 else file_name
            )
            try:
                value = (
                    artifact.value
                    if isinstance(artifact.value, (str, bytes))
                    else artifact.to_text()
                )
                self.file_manager_driver.save_file(
                    os.path.join(dir_name, formatted_file_name), value
                )
