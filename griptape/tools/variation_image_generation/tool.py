...
class VariationImageGenerationTool(BaseImageGenerationTool):
    ...
    @activity(
        config={
            ...
            "schema": Schema(
                {
                    Literal(
                        "prompt", description=BaseImageGenerationTool.PROMPT_DESCRIPTION
                    ): str,
                    Literal(
                        "negative_prompt",
                        description=BaseImageGenerationTool.NEGATIVE_PROMPT_DESCRIPTION,
                    ): str,
                    Literal(
                        "image_file",
    ...
    def image_variation_from_memory(
        self, params: dict[str, dict[str, str]]
    ) -> ImageArtifact | ErrorArtifact:
        prompt = params["values"]["prompt"]
        negative_prompt = params["values"]["negative_prompt"]
        ...
        try:
            image_artifact = load_artifact_from_memory(
                memory, artifact_namespace, artifact_name, ImageArtifact
            )
        except ValueError as e:
            return ErrorArtifact(str(e))

        return self._generate_variation(
            prompt, negative_prompt, cast(ImageArtifact, image_artifact)
        )
