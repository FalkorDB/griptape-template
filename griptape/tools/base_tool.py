...
class BaseTool(ActivityMixin, SerializableMixin, RunnableMixin["BaseTool"], ABC):
    ...
    install_dependencies_on_init: bool = field(
    ...
    def validate_output_memory(
        self, _: Attribute, output_memory: dict[str, Optional[list[TaskMemory]]]
    ) -> None:
        if output_memory:
    ...
    def schema(self) -> dict:
        full_schema = Schema(
            Or(*self.activity_schemas()), description=f"{self.name} action schema."
        )
    ...
    def run(
        self, activity: Callable, subtask: ActionsSubtask, action: ToolAction
    ) -> BaseArtifact:
        try:
    ...
    def to_native_tool_name(self, activity: Callable) -> str:
        ...
        if re.match(r"^[a-zA-Z0-9_]+$", activity_name) is None:
            raise ValueError(
                "Activity name can only contain letters, numbers, and underscores."
            )
